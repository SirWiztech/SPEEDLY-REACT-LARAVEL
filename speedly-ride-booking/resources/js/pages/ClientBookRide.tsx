...
interface Driver { id: string; name: string; rating: number; rides: number; distance: number; vehicle: string; plate: string; type: string; photo?: string; vehicle_image?: string; }
...
const ClientBookRide: React.FC = () => {
    ...
    const calculateFare = useCallback(async (pickupLat: number, pickupLng: number, destLat: number, destLng: number, rideType?: string) => {
        try {
            const type = rideType || booking.plan || 'economy';
            const data = await api.rides.calculateFare({ pickup_lat: pickupLat, pickup_lng: pickupLng, dropoff_lat: destLat, dropoff_lng: destLng, ride_type: type });
            if (data.success && data.data) { setBooking(prev => ({ ...prev, distance: data.data.distance_km || 0, fare: data.data.estimated_fare || 0 })); findNearbyDrivers(pickupLat, pickupLng, type); }
        } catch {}
    }, [booking.plan]);

    const findNearbyDrivers = async (lat: number, lng: number, rideType?: string) => {
        try {
            const data = await api.driver.nearbyDrivers({ lat, lng, radius_km: 10, ride_type: rideType || selectedPlan || 'economy' });
            if (data.success && data.data) { const r = Array.isArray(data.data) ? data.data : []; setDrivers(r.map((d: any) => ({ id: d.id, name: d.user?.full_name || 'Unknown', rating: d.average_rating || 0, rides: d.completed_rides || 0, distance: d.distance ? Math.round(d.distance*10)/10 : 0, vehicle: d.vehicle?.vehicle_type || 'Standard', plate: d.vehicle?.plate_number || '', type: d.vehicle?.vehicle_type || 'standard', photo: d.user?.profile_picture_url || undefined, vehicle_image: d.vehicle?.vehicle_image_url || undefined }))); }
        } catch {}
    };
    ...
    const selectPlan = (plan: string) => { setSelectedPlan(plan); setBooking(prev => { const u = { ...prev, plan }; if (u.pickup.lat && u.pickup.lng && u.destination.lat && u.destination.lng) calculateFare(u.pickup.lat, u.pickup.lng, u.destination.lat, u.destination.lng, plan); return u; }); };
